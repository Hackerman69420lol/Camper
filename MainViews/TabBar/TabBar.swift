...
struct tabBar: View {
    ...
    var body: some View {
        ...
        .onAppear{
            Task {
                //await userData.fetchCurrentUser()
                
                do {
                    try await postData.fetchLikedPosts()
                } catch {
                    // Handle errors from fetchLikedPosts here
                    print("Error fetching liked posts: \(error)")
                }
                
                do {
                    try await postData.fetchRepostedPosts()
                } catch {
                    // Handle errors from fetchRepostedPosts here
                    print("Error fetching reposted posts: \(error)")
                }
                do {
                    try await postData.fetchPublicPosts()
                } catch {
                    // Handle errors from fetchRepostedPosts here
                    print("Error fetching reposted posts: \(error)")
                }
